...
public class SampleDocumentGeneratorTest {
    ...
    public void testGenerateSimplePlainDocument() throws RedPenException {
        ...
        List<Document> doc = SampleDocumentGenerator.generateOneFileDocument(sampleText,
                DocumentParser.PLAIN);
        assertNotNull(doc);
        assertEquals(1, doc.size());
        assertEquals(1, doc.get(0).size());
        assertEquals(1, doc.get(0).getSection(0).getNumberOfParagraphs());
        assertEquals(1, doc.get(0).getSection(0).getParagraph(0)
                .getNumberOfSentences());
        assertEquals("Gekioko pun pun maru means very very angry.", doc.get(0).getSection(0)
    ...
    public void testGenerateSimpleWikiDocument() throws RedPenException {
        ...
        assertNotNull(doc);
        assertEquals(1, doc.size());
        assertEquals(2, doc.get(0).size());
        assertEquals("About Gekioko.", doc.get(0).getSection(1).getHeaderContent(0).content);
        assertEquals(1, doc.get(0).getSection(1).getNumberOfParagraphs());
        assertEquals(1, doc.get(0).getSection(1).getParagraph(0)
                .getNumberOfSentences());
        assertEquals("Gekioko pun pun maru means very very angry.", doc.get(0).getSection(1)
                .getParagraph(0).getSentence(0).content);
    }
